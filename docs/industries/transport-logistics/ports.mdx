...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="crane_qc_07_moves", name="Crane QC-07 moves", unit="count", value_type="float")])
...
let mut congestion = Event::new(
    format!("yard_congestion_b07_{}", Utc::now().timestamp()),
    "yard_congestion".into(),
    Utc::now());
...
rf = intellistream_datahub_sdk.RetrieveFilter(ts="crane_qc_07_moves",
