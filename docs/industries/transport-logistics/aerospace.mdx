...
New workspace? Run this once (Python) to create the part type and two aircraft carrying it;
step 1 below adds the third, so the blast-radius walk in step 2 returns the whole affected
fleet. Safe to re-run.
...
```java
Resource aircraft = new Resource();
...
client.resources.create(
    [intellistream_datahub_sdk.Resource(external_id="aircraft_ln_312", name="Aircraft LN-312", labels=["aircraft_ln_312"]),
     intellistream_datahub_sdk.Resource(external_id="hyd_pump_ln_312_1", name="Hydraulic pump #1", labels=["hyd_pump_ln_312_1"])],
    [intellistream_datahub_sdk.RelForm.by_external_ids("aircraft_ln_312", "hyd_pump_ln_312_1", "contains"),
     intellistream_datahub_sdk.RelForm.by_external_ids("hyd_pump_ln_312_1", "part_type_hp_47", "is_part_type")])
```
