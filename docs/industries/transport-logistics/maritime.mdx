...
for s in ["reefer_msc_1182_supply_c", "reefer_msc_1182_setpoint_c"]:
    client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id=s, name=s, unit="deg_c", value_type="float")])
...
while let Some(Ok(msg)) = listener.next().await {
    if out_of_band(&msg.payload) {                // |supply - setpoint| > tolerance
        let mut excursion = Event::new(
            format!("reefer_excursion_1182_{}", Utc::now().timestamp()),
            "reefer_excursion".into(),
            Utc::now());
