...
desk_id = client.datasets.create([intellistream_datahub_sdk.Dataset(external_id="desk_equities", name="Equities desk")])[0].id
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="portfolio_growth_mtm_usd",
...
created = client.datasets.create([
    intellistream_datahub_sdk.Dataset(external_id="desk_equities", name="Equities desk")])
...
```java
var mtm = new Timeseries();
mtm.setExternalId("portfolio_growth_mtm_usd");
mtm.setName("Growth portfolio — mark-to-market (USD)");
mtm.setValueType("numeric");
mtm.setDataSetId(deskId);
mtm.setUnit("usd");
...
ts.value_type = Some("numeric".into());
...
let mut breach = Event::new(
    format!("var_limit_breach_growth_{}", Utc::now().timestamp()),
    "var_limit_breach".into(),
    Utc::now());
...
rf = intellistream_datahub_sdk.RetrieveFilter(ts="portfolio_growth_mtm_usd",
