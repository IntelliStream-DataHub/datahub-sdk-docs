...
New workspace? Run this once (Python) to create the ward, the pump model and two of its
pumps; step 1 below adds the third, so the recall walk in step 2 returns the full pull list.
Safe to re-run.
...
nodes = [intellistream_datahub_sdk.Resource(external_id="model_acme_x200", name="Acme X200 infusion pump", labels=["PumpModel"]),
         intellistream_datahub_sdk.Resource(external_id="ward_icu", name="Intensive care", labels=["Ward"])]
edges = []
for u in ["3392", "3401"]:
    nodes.append(intellistream_datahub_sdk.Resource(external_id=f"pump_icu_{u}", name=f"Infusion pump {u}", labels=["Pump"]))
    edges += [intellistream_datahub_sdk.RelForm.by_external_ids(f"pump_icu_{u}", "ward_icu", "located_in"),
              intellistream_datahub_sdk.RelForm.by_external_ids(f"pump_icu_{u}", "model_acme_x200", "is_model")]
...
client.resources.create(
    [intellistream_datahub_sdk.Resource(external_id="pump_icu_3391", name="Infusion pump 3391", labels=["Pump"])],
    [intellistream_datahub_sdk.RelForm.by_external_ids("pump_icu_3391", "ward_icu", "located_in"),
     intellistream_datahub_sdk.RelForm.by_external_ids("pump_icu_3391", "model_acme_x200", "is_model")])
```
