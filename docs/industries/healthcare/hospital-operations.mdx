...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="ward_icu_occupancy_pct", name="ICU occupancy %", unit="pct", value_type="float")])
...
client.resources.create(
    [intellistream_datahub_sdk.Resource(external_id="hospital_central", name="Central Hospital", labels=["Hospital"]),
     intellistream_datahub_sdk.Resource(external_id="ward_icu", name="Intensive care", labels=["Ward"])],
    [intellistream_datahub_sdk.RelForm.by_external_ids("hospital_central", "ward_icu", "contains")])
```
...
while let Some(Ok(msg)) = listener.next().await {
    ...
    if over_threshold(&msg.payload) {                  // e.g. occupancy > 95%
        let mut warning = Event::new(
            format!("capacity_warning_icu_{}", Utc::now().timestamp()),
            "capacity_warning".into(),
            Utc::now());
