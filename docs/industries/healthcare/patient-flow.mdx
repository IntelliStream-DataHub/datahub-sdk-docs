...
New workspace? Run this once (Python) to create the ED time-to-bed series with a day of
readings whose wait grows through the afternoon. Safe to re-run.
...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="ed_time_to_bed_minutes", name="ED time to bed (min)", unit="min", value_type="float")])
...
let mut breach = Event::new(
    format!("flow_breach_ed_{}", Utc::now().timestamp()),
    "flow_breach".into(),
    Utc::now());
...
rf = intellistream_datahub_sdk.RetrieveFilter(ts="ed_time_to_bed_minutes",
