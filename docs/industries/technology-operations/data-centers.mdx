...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="rack_r14_inlet_temp_c", name="Rack R14 inlet temp", unit="deg_c", value_type="float")])
...
client.resources.create(
    ...
    [intellistream_datahub_sdk.RelForm.by_external_ids("rack_r14", "crac_unit_2", "cooled_by"),
     intellistream_datahub_sdk.RelForm.by_external_ids("rack_r15", "crac_unit_2", "cooled_by")])
```
...
while let Some(Ok(msg)) = listener.next().await {
    if inlet_above(&msg.payload, 27.0) {              // °C — above ASHRAE band
        let mut warning = Event::new(
            format!("thermal_warning_r14_{}", Utc::now().timestamp()),
            "thermal_warning".into(),
            Utc::now());
...
Set<String> aNodes = a.nodes().stream().map(NodeModel::getExternalId).collect(Collectors.toSet());
Set<String> shared = b.nodes().stream().map(NodeModel::getExternalId)
...
let an: HashSet<&str> = a.nodes().iter().map(|n| n.external_id()).collect();
let shared: Vec<&str> = b.nodes().iter()
    .map(|n| n.external_id()).filter(|id| an.contains(id)).collect();
