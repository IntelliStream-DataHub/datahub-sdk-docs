...
client.timeseries.create([datahub_sdk.TimeSeries(external_id="rack_r14_inlet_temp_c", name="Rack R14 inlet temp")])
...
client.resources.create(
    [datahub_sdk.Resource(external_id=x, name=x) for x in ["rack_r14", "rack_r15", "crac_unit_2"]],
...
try (SubscriptionListener listener =
         client.subscriptions().listen(List.of("hall_2_thermal"))) {
    while (running) {
...
while let Some(Ok(msg)) = listener.next().await {
    if inlet_above(&msg.payload, 27.0) {              // °C — above ASHRAE band
        let mut warning = Event::new("thermal_warning_r14".into());
