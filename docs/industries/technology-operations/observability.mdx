...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="service_checkout_request_latency_ms", name="Checkout p99 latency", unit="ms", value_type="float")])
...
client.resources.create(
    ...
    [intellistream_datahub_sdk.RelForm.by_external_ids("service_checkout", "order_db", "depends_on"),
     intellistream_datahub_sdk.RelForm.by_external_ids("service_payments", "order_db", "depends_on"),
     intellistream_datahub_sdk.RelForm.by_external_ids("service_checkout", "host_web_01", "runs_on")])
```
...
client.resources.create(
    [intellistream_datahub_sdk.Resource(external_id="service_checkout", name="Checkout service", labels=["Service"]),
     intellistream_datahub_sdk.Resource(external_id="host_web_01", name="web-01", labels=["Host"])],
    [intellistream_datahub_sdk.RelForm.by_external_ids("service_checkout", "host_web_01", "runs_on")])
```
...
while let Some(Ok(msg)) = listener.next().await {
    if breaches_slo(&msg.payload) {           // e.g. p99 latency > 300 ms
        let mut event = Event::new(
            format!("slo_breach_checkout_{}", Utc::now().timestamp()),
            "slo_breach".into(),
            Utc::now());
...
retriever.getFilter().setType(List.of("slo_breach"));
...
```python
breaches = client.events.filter(intellistream_datahub_sdk.EventFilter(
    basic_filter=intellistream_datahub_sdk.BasicEventFilter(type="slo_breach"), limit=200))
```
...
Set<String> shared = payments.nodes().stream()
    .map(NodeModel::getExternalId).filter(checkoutDeps::contains).collect(Collectors.toSet());
...
let deps: HashSet<&str> = checkout.nodes().iter().map(|n| n.external_id()).collect();
let shared: Vec<&str> = payments.nodes().iter()
    .map(|n| n.external_id()).filter(|id| deps.contains(id)).collect();
// shared contains "order_db" → one failing dependency, not two outages
```
