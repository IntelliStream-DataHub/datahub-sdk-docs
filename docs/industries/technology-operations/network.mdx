...
New workspace? Run this once (Python) to create two cells' utilisation series (with
busy-hour peaks) and the topology where they share a backhaul link — so the localisation
in step 2 points at the link. Safe to re-run.
...
for c in ["cell_oslo_4412", "cell_oslo_4418"]:
    client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id=f"{c}_prb_util", name=f"{c} PRB util", unit="pct", value_type="float")])
...
client.resources.create(
    ...
    [intellistream_datahub_sdk.RelForm.by_external_ids("cell_oslo_4412", "backhaul_link_88", "backhauled_by"),
     intellistream_datahub_sdk.RelForm.by_external_ids("cell_oslo_4418", "backhaul_link_88", "backhauled_by"),
     intellistream_datahub_sdk.RelForm.by_external_ids("backhaul_link_88", "controller_rnc_3", "connects_to")])
```
...
client.timeseries().retrieve(request).getItems().iterator().next().getDatapoints()
...
Set<String> n1 = c1.nodes().stream().map(NodeModel::getExternalId).collect(Collectors.toSet());
Set<String> shared = c2.nodes().stream().map(NodeModel::getExternalId)
...
let n1: HashSet<&str> = c1.nodes().iter().map(|n| n.external_id()).collect();
let shared: Vec<&str> = c2.nodes().iter()
    .map(|n| n.external_id()).filter(|id| n1.contains(id)).collect();
