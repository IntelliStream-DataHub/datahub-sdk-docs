...
New workspace? Run this once (Python) to create a haul truck's oil-pressure series (sagging
below its limit) and a crusher throughput series. Safe to re-run.
...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="truck_785_oil_pressure_kpa", name="Truck 785 oil pressure", unit="kpa", value_type="float")])
...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="crusher_01_throughput_tph", name="Crusher 01 throughput", unit="tph", value_type="float")])
...
let low = points.datapoints.iter()
    .any(|p| p.value.unwrap_or(999.0) < 250.0);
if low {
    let mut warning = Event::new(
        format!("equipment_warning_785_{}", Utc::now().timestamp()),
        "equipment_warning".into(),
        Utc::now());
...
- **[Predictive maintenance (advanced)](/advanced/predictive-maintenance)** — anomaly detection on haul-truck vibration.
