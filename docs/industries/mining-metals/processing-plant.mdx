...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="mill_1_recovery_pct", name="Mill 1 recovery %", unit="pct", value_type="float")])
...
let mut loss = Event::new(
    format!("recovery_loss_mill1_{}", Utc::now().timestamp()),
    "recovery_loss".into(),
    Utc::now());
...
rf = intellistream_datahub_sdk.RetrieveFilter(ts="mill_1_recovery_pct",
