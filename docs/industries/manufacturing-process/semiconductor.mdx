...
client.resources.create(
    ...
    [intellistream_datahub_sdk.RelForm.by_external_ids("lot_22841", "tool_etch_07", "processed_on"),
     intellistream_datahub_sdk.RelForm.by_external_ids("lot_22841", "tool_litho_03", "processed_on"),
     intellistream_datahub_sdk.RelForm.by_external_ids("lot_22863", "tool_etch_07", "processed_on"),
     intellistream_datahub_sdk.RelForm.by_external_ids("lot_22863", "tool_cmp_05", "processed_on")])
```
...
pressure.setExternalId("tool_etch_07_chamber_pressure_mtorr");
pressure.setName("Etch-07 chamber pressure");
pressure.setValueType("numeric");
...
p.value_type = Some("numeric".into());
...
Set<String> toolsA = lotA.nodes().stream()
    .map(NodeModel::getExternalId).filter(id -> id.startsWith("tool_")).collect(Collectors.toSet());
Set<String> suspect = lotB.nodes().stream()
    .map(NodeModel::getExternalId).filter(toolsA::contains).collect(Collectors.toSet());
...
let tools_a: HashSet<&str> = lot_a.nodes().iter()
    .map(|n| n.external_id()).filter(|id| id.starts_with("tool_")).collect();
let suspect: Vec<&str> = lot_b.nodes().iter()
    .map(|n| n.external_id()).filter(|id| tools_a.contains(id)).collect();
