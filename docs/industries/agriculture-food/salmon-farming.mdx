...
try (SubscriptionListener listener =
         client.subscriptions().listen(List.of("farm_hardanger_pens"))) {
    while (running) {
...
while let Some(Ok(msg)) = listener.next().await {
    if oxygen_below(&msg.payload, 6.0) {              // mg/L — danger approaching
        let mut alarm = Event::new("low_oxygen_h07".into());
...
client.timeseries().create(
    Timeseries.of("pen_h_07_feed_kg").name("Pen H-07 feed dispensed").unit("kg"),
