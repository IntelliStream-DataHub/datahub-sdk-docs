...
double monthly = client.timeseries().retrieve(request).getItems().iterator().next()
...
let mut exceedance = Event::new(
    format!("flaring_exceedance_north_{}", Utc::now().timestamp()),
    "flaring_exceedance".into(),
    Utc::now());
...
rf = intellistream_datahub_sdk.RetrieveFilter(ts="platform_north_flare_volume_m3",
