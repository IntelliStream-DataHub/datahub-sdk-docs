...
New workspace? Run this once (Python) to create the well's sensor series and feed in a
pump-intake pressure that sags toward the anomaly threshold — so the alarm in step 3
actually fires. The field/well/pump graph is created by step 1 below; this also adds the
two temperature sensors and the shared `cooling_system` that step 4 walks between, so the
correlation there returns something. Safe to re-run.
...
for s, u in sensors.items():
    client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id=s, name=s, unit=u, value_type="float")])
...
# step 4 walks out from two alarmed temperature sensors and looks for what they share
client.resources.create(
    [intellistream_datahub_sdk.Resource(external_id=x, name=n, labels=[lbl]) for x, n, lbl in
     [("cooling_system", "Platform cooling system", "System"),
      ("wellhead_temp_a12", "Wellhead temperature A-12", "Sensor"),
      ("pump_temp_a12", "ESP motor temperature A-12", "Sensor")]],
    [intellistream_datahub_sdk.RelForm.by_external_ids("wellhead_temp_a12", "cooling_system", "PART_OF"),
     intellistream_datahub_sdk.RelForm.by_external_ids("pump_temp_a12", "cooling_system", "PART_OF")])
```
...
nodes = [
    intellistream_datahub_sdk.Resource(external_id="field_north_sea", name="North Sea field", labels=["Field"]),
    intellistream_datahub_sdk.Resource(external_id="well_a12", name="Well A-12", labels=["Well"]),
    intellistream_datahub_sdk.Resource(external_id="pump_esp_a12", name="ESP — Well A-12", labels=["Pump"]),
]
edges = [
    intellistream_datahub_sdk.RelForm.by_external_ids("field_north_sea", "well_a12", "contains"),
    intellistream_datahub_sdk.RelForm.by_external_ids("well_a12", "pump_esp_a12", "contains"),
]
...
let low = points.datapoints.iter()
    .any(|p| p.value.unwrap_or(99.0) < 80.0);
if low {
    let mut event = Event::new(
        format!("esp_anomaly_a12_{}", Utc::now().timestamp()),
        "esp_anomaly".into(),
        Utc::now());
...
Set<String> shared = nb.nodes().stream()
    .map(NodeModel::getExternalId).filter(aNodes::contains).collect(Collectors.toSet());
...
let a: HashSet<&str> = na.nodes().iter().map(|n| n.external_id()).collect();
let shared: Vec<&str> = nb.nodes().iter()
    .map(|n| n.external_id()).filter(|id| a.contains(id)).collect();
// shared contains "cooling_system" → one root cause, not two
```
...
rf = intellistream_datahub_sdk.RetrieveFilter(ts="pump_intake_pressure_bar",
