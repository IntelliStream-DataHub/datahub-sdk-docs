...
New workspace? Run this once (Python) to create a crane-utilisation series; step 1 below
models the site itself. (The drawing upload there needs a local file, any PDF works.) Safe to
re-run.
...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="crane_02_hours", name="Crane 02 hours", unit="h", value_type="float")])
...
client.resources.create(
    [intellistream_datahub_sdk.Resource(external_id="site_harbour_tower", name="Harbour Tower", labels=["Site"]),
     intellistream_datahub_sdk.Resource(external_id="zone_level_12", name="Level 12", labels=["Zone"])],
    [intellistream_datahub_sdk.RelForm.by_external_ids("site_harbour_tower", "zone_level_12", "contains")])
...
let mut incident = Event::new(
    format!("safety_incident_l12_{}", Utc::now().timestamp()),
    "safety_incident".into(),
    Utc::now());
