...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id="bin_grunerlokka_114_fill_pct", name="Bin 114 fill", unit="pct", value_type="float")])
...
let points = api.time_series.retrieve_datapoints(&last_reading("bin_grunerlokka_114_fill_pct")).await?
    .get_items().remove(0);
let fill: f64 = points.datapoints[0].value.unwrap_or(0.0);
if fill >= 80.0 {
    let mut due = Event::new(
        format!("collection_due_114_{}", Utc::now().timestamp()),
        "collection_due".into(),
        points.datapoints[0].timestamp);
