...
client.resources.create(
    ...
    [intellistream_datahub_sdk.RelForm.by_external_ids("source_lake_a", "main_trunk_north", "feeds"),
     intellistream_datahub_sdk.RelForm.by_external_ids("main_trunk_north", "zone_west_12", "feeds"),
     intellistream_datahub_sdk.RelForm.by_external_ids("main_trunk_north", "zone_east_07", "feeds")])

for s in ["zone_west_12_turbidity_ntu", "zone_east_07_turbidity_ntu"]:
    client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id=s, name=s, unit="ntu", value_type="float")])
...
Set<String> common = east.nodes().stream()
    .map(NodeModel::getExternalId).filter(westUpstream::contains).collect(Collectors.toSet());
...
let w: HashSet<&str> = west.nodes().iter().map(|n| n.external_id()).collect();
let common: Vec<&str> = east.nodes().iter()
    .map(|n| n.external_id()).filter(|id| w.contains(id)).collect();
