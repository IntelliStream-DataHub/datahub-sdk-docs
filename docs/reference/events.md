...
| `setNull: true` | nullable fields only | Clear the value. `type` is required, so asking to clear it is a `400`. |
...
An event's identity is immutable. `id`, `externalId` and `eventTime` are absent from the
update form entirely, and sending any of them inside `update` is a `400` naming the field.
`eventTime` cannot move because the store partitions by it, and `externalId` is the
correlation key that groups a subject's history (see [External ids](./external-ids)), so
renaming one event would tear it out of its own audit trail. To re-key an event, create a
new one and delete the old.

`setNull` is refused on `type`: clearing it used to be accepted, and it left the event
unreadable by any client that models `type` as required, so the read failed rather than the
write. `dataSetId` is the one field here that genuinely is nullable: `setNull` detaches the
event from its data set, and naming a `dataSetId` that no data set has is a `400` rather
than a stored dangling reference.
