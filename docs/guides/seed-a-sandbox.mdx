...
Every block below can be run again. A second run of a series or resource create answers
`409` on the duplicate external id: the helper catches that and goes on to write the
datapoints, which dedup on `(series, timestamp)`, and block B catches it around the graph
create. Events are the exception: an event's external id is not unique, so a second run raises
two more alarms.
...
try:
    ...
    if e.status_code != 409:              # a second run finds the graph already there
        raise
```
...
alarms = client.events.filter(type="alarm", external_id="alarm_sensor_*", limit=10)

print(f"{len(points)} readings in the last hour, {len(alarms)} alarms")
# 60 readings in the last hour, 2 alarms (2 more for each re-run of the alarm block)
```
