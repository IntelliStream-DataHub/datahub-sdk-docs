...
```python
overheats = client.events.filter(intellistream_datahub_sdk.EventFilter(
    basic_filter=intellistream_datahub_sdk.BasicEventFilter(type="overheat"),
    limit=100))
```
...
let filter = EventFilter::default()
    .set_filter(BasicEventFilter { r#type: Some(vec!["overheat".into()]), ..Default::default() })
    .set_limit(100)
    .build();
