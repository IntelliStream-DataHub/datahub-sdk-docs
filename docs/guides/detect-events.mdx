...
if too_hot {
    let mut event = Event::new("overheat_press_07".into());
...
// narrow by type / time range / metadata on the retriever
...
let overheats = api.events.filter(&EventFilter::default()).await?;
```
