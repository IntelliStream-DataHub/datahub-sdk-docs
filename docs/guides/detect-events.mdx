...
var series = client.timeseries().retrieve(request).getItems().iterator().next();
...
if any(dp.value is not None and dp.value > 110.0 for dp in points):
...
let response = api.time_series
    .retrieve_datapoints(&DataWrapper::from_vec(vec![filter])).await?;
let series = response.get_items().first().ok_or("engine_temperature returned nothing")?;

let too_hot = series.datapoints.iter()
    .any(|p| p.value.is_some_and(|v| v > 110.0));

if too_hot {
    let mut event = Event::new(
        format!("overheat_press_07_{}", Utc::now().timestamp()),
        "overheat".into(),
        Utc::now());
...
retriever.getFilter().setType(List.of("overheat"));
...
```python
overheats = client.events.filter(type="overheat", limit=100)
```
...
let filter = EventFilterForm::default()
    .set_filter(EventFilter { r#type: Some(vec!["overheat".into()]), ..Default::default() })
    .set_limit(100)
    .build();
