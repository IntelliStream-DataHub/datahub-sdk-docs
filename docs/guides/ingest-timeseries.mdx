...
and both retried for you. See [Limits & quotas](/reference/limits). Java's
[binary path](#binary-ingest) has caps of its own, and the SDK stays under them by itself.
:::
...
to parse. `ingestBinary` sends the same map as zstd-compressed Arrow frames to
[`POST /timeseries/data/binary`](/reference/binary-datapoints), up to a million points per
request, parsed and sorted on the client. Pick it when the volume is sustained; keep `ingest`
when you want the [durable spool](/reference/client#durable-ingest-buffering), which the binary
path does not have. It resolves each series through `/timeseries/byids` once and caches it, so
the token needs **read** access to the data set as well as write.
...
A series that does not exist, or a value that does not fit its type, is reported in
`result.errors()` without a request (status 404 and 422). A request the server refuses because a
series was renamed or removed since it was cached is rebuilt once after re-resolving.

For readings that arrive one at a time, a buffer batches them into frames and sends once 10 000
points have accumulated or the oldest is 200 ms old. `add` never blocks on the network, and
closing the buffer flushes what is left:
...
The options and the buffer's other methods are in the
