...
resolve the unit's symbol, quantity and conversion. The unit leaves the value type alone (the Python
and Rust examples set `float`); it describes what the numbers mean.
...
series.setExternalId("wellhead_pressure_bar");
series.setName("Wellhead pressure");
series.setUnit("bar");
...
`byIds` takes `IdCollection`, by external id or by numeric id:

```java
UnitModel celsius = client.units().byIds(List.of(
        IdCollection.createFromExternalId("temperature_deg_c"))).getItems().iterator().next();
UnitModel byId = client.units().byIds(List.of(
        IdCollection.createFromId(7))).getItems().iterator().next();
```
