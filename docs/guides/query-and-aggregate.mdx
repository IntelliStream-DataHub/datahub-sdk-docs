...
for series in result.get_items() {
    for p in &series.datapoints {
        println!("{} = {:?}", p.timestamp, p.value);     // value is Option<f64>
    }
}
```
...
// NOTE: Java can't read aggregate values yet — getValue() returns null here (see warning below)
...
do {
    ...
    var page = client.timeseries().retrieve(request).getItems().iterator().next();
...
cursor = None
while True:
    rf = intellistream_datahub_sdk.RetrieveFilter(ts="engine_temperature",
