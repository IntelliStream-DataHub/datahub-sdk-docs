...
println!("{} nodes, {} edges",
    graph.nodes().map_or(0, |n| n.len()),
    graph.relations().map_or(0, |r| r.len()));
```
...
oilTemp.setExternalId("press_07_oil_temp");
oilTemp.setName("Press 07 oil temperature");
...
client.timeseries.create([intellistream_datahub_sdk.TimeSeries(
    external_id="press_07_oil_temp", name="Press 07 oil temperature", value_type="float",
...
oil_temp.value_type = Some("float".into());
...
Fetch a node's neighbourhood with `fetch_related` (`fetchRelated` in Java); the returned network
carries the nodes it found and the edges between them. (`by_ids` returns the node alone:
`relatedResources` is empty on that read.) One hop out from the press finds the two edges
created above.
