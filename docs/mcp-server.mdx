...
For an unattended agent, get the token from the OAuth2 **client-credentials** grant, and
include `scope=organization:*` in the token request, exactly as for programmatic REST
access: the organization claim that check 3 reads is only issued when that scope is
requested.
