...
| `timeseries_fetch_datapoints` | History between two timestamps, ISO-8601 or an epoch |
...
| `start`, `end` | Window, ISO-8601 or an epoch. Required |
...
- **Timestamps match the REST API**, so a value reads the same whichever way the agent reached
  it. They come back as ISO-8601 strings, and a timestamp argument takes ISO-8601 or an epoch on
  the same [seconds-or-millis rule](/reference/timeseries#write-datapoints).
