...
| --- | --- | --- |
| API | `<api-url>/mcp` (port 8081 locally) | **37 tools** across data sets, resources, relationships, time series, events, labels and units |
...
| Tool | What it does |
| --- | --- |
| `resource_search` | Full-text search over name, externalId and description, across every node type, ranked by relevance |
...
| `timeseries_fetch_datapoints` | History between two timestamps, ISO-8601 or epoch millis |
...
| Tool | What it does |
| --- | --- |
| `event_search` | Substring search over externalId, description and metadata values, newest first |
...
| `start`, `end` | Window, ISO-8601 or epoch millis. Required |
...
- **Timestamps match the REST API**, so a value reads the same whichever way the agent reached
  it. They come back as ISO-8601 strings, and a timestamp argument takes ISO-8601 or epoch
  milliseconds, on the same [rules](/reference/timeseries#write-datapoints).
