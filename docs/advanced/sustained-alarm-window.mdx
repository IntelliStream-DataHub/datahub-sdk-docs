...
var points = client.timeseries().retrieve(request)
        .getItems().iterator().next().getDatapoints();
...
points = client.timeseries.retrieve_datapoints(intellistream_datahub_sdk.RetrieveFilter(
    ...
    end=pd.Timestamp.now(tz="UTC"), limit=10_000))[0].get_datapoints()
...
for p in &series.datapoints {
    let at = p.timestamp;
    let value = p.value.unwrap_or(0.0);
...
```python
found = list(client.events.filter(type="sustained_high", limit=100))
