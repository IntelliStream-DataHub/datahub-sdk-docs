...
that's too late — by the time dissolved oxygen in a [salmon pen](/industries/aquaculture)
...
def load(external_id, days=90):
    rf = datahub_sdk.RetrieveFilter(
        ...
        aggregates=["average"], granularity="5m")
...
alarm](/industries/aquaculture) would ever fire — so aeration starts in time.
...
- [Aquaculture — salmon farming](/industries/aquaculture) — the reactive version this upgrades.
