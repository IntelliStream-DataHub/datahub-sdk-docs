...
def series(tag, start, end):
    rf = intellistream_datahub_sdk.RetrieveFilter(ts=tag, start=start, end=end,
...
filter.setEnd(ZonedDateTime.now().minusDays(1));
...
var pts = client.timeseries().retrieve(request).getItems().iterator().next().getDatapoints();
```
...
found = list(client.events.filter(type="process_deviation", limit=10))
