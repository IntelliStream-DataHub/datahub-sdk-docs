...
def load(ext_id):
    rf = intellistream_datahub_sdk.RetrieveFilter(ts=ext_id,
...
def store(ext_id, series, name=None, unit="value"):
    client.timeseries.create([intellistream_datahub_sdk.TimeSeries(external_id=ext_id, name=name or ext_id, unit=unit,
                                                     value_type="float",
...
```python
def fn(ext_id, name):
    return intellistream_datahub_sdk.Resource(external_id=ext_id, name=name, labels=["PipelineStep"])
...
def proc(src, f): return intellistream_datahub_sdk.RelForm.by_external_ids(src, f, "processed_by")
def prod(f, out): return intellistream_datahub_sdk.RelForm.by_external_ids(f, out, "produces")
