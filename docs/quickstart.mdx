...
The SDK targets Java 25, so the consuming project needs a JDK 25 toolchain.
...
All three SDKs are Apache-2.0, so you can link them into a closed-source application. The
DataHub platform itself is AGPL-3.0; the client libraries are deliberately not.
