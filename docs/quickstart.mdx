...
release. The SDKs are pre-1.0, so a release can still rename or remove a call. If an example
calls something your installed SDK does not have, it may be newer than your release.

| SDK | Latest release |
| --- | --- |
| Python | 0.2.0, on PyPI |
| Rust | 0.2.0, on crates.io |
| Java | Not released yet, so it is built from the platform repository below |
:::
...
Then depend on it. The version is `javaSdkVersion` in the platform's `gradle.properties`,
`0.3.0-SNAPSHOT` on its default branch:
...
client = DataHubClient.from_envfile(None)  # the environment, plus a .env file if there is one
...
client.timeseries.insert_from_lists(
    timestamps=[datetime.now(timezone.utc)], values=[92.4], ts="engine_temperature")
```

</TabItem>
<TabItem value="rust" label="Rust">

Every call is `async`, so run these lines inside
`#[tokio::main] async fn main() -> Result<(), Box<dyn std::error::Error>>`, with
`tokio = { version = "1", features = ["full"] }` and `chrono = "0.4"` added to your
dependencies.
